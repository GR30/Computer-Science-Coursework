...
GLuint startCube = 0, startAxes = 0, startSquare = 0, startOcta, startTetra;
...
color4 material_ambient(0.0, 0.1, 0.06, 1.0 );
...
ParticleSystem::ParticleSystem(int nParticles, Material newMat)
{
	...
	particles->transform.position.x = (GLfloat)(rand() % 100) / 50.0f - 0.5f;
	particles->transform.position.y = (GLfloat)(rand() % 100) / 50.0f - 0.5f;
	particles->transform.position.z = (GLfloat)(rand() % 100) / 50.0f - 0.5f;
	particles->velocity = 0; //vec3((GLfloat)(rand() % 10) / 100.0f - 0.05f, (GLfloat)(rand() % 10) / 100.0f - 0.05f, (GLfloat)(rand() % 10) / 100.0f - 0.05f);
	particles->rotationSpeed = vec3(0, 10, 0);
	particles->transform.scale = vec3(0.4, 0.4, 0.4);
	for(int i = 1; i < nParticles; i++)
	{
		Particle *tempParticle = new Particle();
		tempParticle->transform.position.x += 0; //(GLfloat)(rand() % 100) / 100.0f - 0.5f;
		tempParticle->transform.position.y += 0; //(GLfloat)(rand() % 100) / 100.0f - 0.5f;
		tempParticle->transform.position.z += 0; //(GLfloat)(rand() % 100) / 100.0f - 0.5f;
...
ParticleSystem *onePS, *twoPS, *threePS;
...
void computeOcta(vec4 *thePoints, vec3 *theNormals, vec3 *theTangents, vec2 *theTexCoords, int *index) {
	thePoints[*index] = vec4(-1,  0,  0,  1.0);
	thePoints[*index + 1] = vec4( 0,  0,  1, 1.0);
	thePoints[*index + 2] = vec4( 0,  1,  0, 1.0);
	thePoints[*index + 3] = vec4( 1,  0,  0, 1.0);
	thePoints[*index + 4] = vec4( 0,  0,  1, 1.0);
	thePoints[*index + 5] = vec4( 0,  1,  0, 1.0);
	thePoints[*index + 6] = vec4( 0,  0, -1, 1.0);
	thePoints[*index + 7] = vec4( 1,  0,  0, 1.0);
	thePoints[*index + 8] = vec4( 0,  1,  0, 1.0);
	thePoints[*index + 9] = vec4(-1,  0,  0, 1.0);
	thePoints[*index + 10] = vec4( 0,  0, -1, 1.0);
	thePoints[*index + 11] = vec4( 0,  1,  0, 1.0);
	thePoints[*index + 12] = vec4(-1, 0, 0, 1.0);
	thePoints[*index + 13] = vec4(0, 0, 1, 1.0);
	thePoints[*index + 14] = vec4(0, -1, 0, 1.0);
	thePoints[*index + 15] = vec4(1, 0, 0, 1.0);
	thePoints[*index + 16] = vec4(0, 0, 1, 1.0);
	thePoints[*index + 17] = vec4(0, -1, 0, 1.0);
	thePoints[*index + 18] = vec4(0, 0, -1, 1.0);
	thePoints[*index + 19] = vec4(1, 0, 0, 1.0);
	thePoints[*index + 20] = vec4(0, -1, 0, 1.0);
	thePoints[*index + 21] = vec4(-1, 0, 0, 1.0);
	thePoints[*index + 22] = vec4(0, 0, -1, 1.0);
	thePoints[*index + 23] = vec4(0, -1, 0, 1.0);
	...
	theNormals[*index + 6] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 7] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 8] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 9] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 10] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 11] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 12] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 13] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 14] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 15] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 16] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 17] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 18] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 19] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 20] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 21] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 22] = vec3(0.0, 0.0, 1.0);
	theNormals[*index + 23] = vec3(0.0, 0.0, 1.0);
	...
	theTangents[*index + 6] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 7] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 8] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 9] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 10] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 11] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 12] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 13] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 14] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 15] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 16] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 17] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 18] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 19] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 20] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 21] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 22] = vec3(0.0, 1.0, 0.0);
	theTangents[*index + 23] = vec3(0.0, 1.0, 0.0);

	theTexCoords[*index] = vec2(1.0, 0.0);
	theTexCoords[*index + 1] = vec2(1.0, 1.0);
	theTexCoords[*index + 2] = vec2(0.0, 0.0);
	theTexCoords[*index + 3] = vec2(1.0, 0.0);
	theTexCoords[*index + 4] = vec2(1.0, 1.0);
	theTexCoords[*index + 5] = vec2(0.0, 0.0);
	theTexCoords[*index + 6] = vec2(1.0, 0.0);
	theTexCoords[*index + 7] = vec2(1.0, 1.0);
	theTexCoords[*index + 8] = vec2(0.0, 0.0);
	theTexCoords[*index + 9] = vec2(1.0, 0.0);
	theTexCoords[*index + 10] = vec2(1.0, 1.0);
	theTexCoords[*index + 11] = vec2(0.0, 0.0);
	theTexCoords[*index + 12] = vec2(1.0, 0.0);
	theTexCoords[*index + 13] = vec2(1.0, 1.0);
	theTexCoords[*index + 14] = vec2(0.0, 0.0);
	theTexCoords[*index + 15] = vec2(1.0, 0.0);
	theTexCoords[*index + 16] = vec2(1.0, 1.0);
	theTexCoords[*index + 17] = vec2(0.0, 0.0);
	theTexCoords[*index + 18] = vec2(1.0, 0.0);
	theTexCoords[*index + 19] = vec2(1.0, 1.0);
	theTexCoords[*index + 20] = vec2(0.0, 0.0);
	theTexCoords[*index + 21] = vec2(1.0, 0.0);
	theTexCoords[*index + 22] = vec2(1.0, 1.0);
	theTexCoords[*index + 23] = vec2(0.0, 0.0);

	*index += 23;
}

void computeTetra(vec4 *thePoints, vec3 *theNormals, vec3 *theTangents, vec2 *theTexCoords, int *index){
	thePoints[*index] = vec4(1.0, 1.0, 1.0, 1.0);
	thePoints[*index + 1] = vec4(-1.0,  1.0, -1.0, 1.0);
	thePoints[*index + 2] = vec4( 1.0, -1.0, -1.0, 1.0);
	thePoints[*index + 3] = vec4( 1.0, -1.0, -1.0, 1.0);
	thePoints[*index + 4] = vec4( 1.0,  1.0,  1.0, 1.0);
	thePoints[*index + 5] = vec4(-1.0, -1.0,  1.0, 1.0);
	thePoints[*index + 6] = vec4( 1.0,  1.0,  1.0, 1.0);
	thePoints[*index + 7] = vec4(-1.0, -1.0,  1.0, 1.0);
	thePoints[*index + 8] = vec4(-1.0,  1.0, -1.0, 1.0);
	thePoints[*index + 9] = vec4( 1.0, -1.0, -1.0, 1.0);
	thePoints[*index + 10] = vec4(-1.0,  1.0, -1.0, 1.0);
	thePoints[*index + 11] = vec4(-1.0, -1.0,  1.0, 1.0);
...
init( void )
{
	...
	startOcta = limit;
	computeOcta(points, normals, tangents, texCoords, &limit);

	startTetra = limit;
	computeTetra(points, normals, tangents, texCoords, &limit);

	onePS = new ParticleSystem(1, gold);
	twoPS = new ParticleSystem(1, ruby);
	threePS = new ParticleSystem(1, emerald);
	...
	*/
	...
	twoPS->setShader(programs[3]);
	threePS->setShader(programs[3]);
...
drawOcta( void ) 
{ 
	glDrawArrays(GL_TRIANGLE_STRIP, startOcta, 3);
	glDrawArrays(GL_TRIANGLE_STRIP, startOcta + 3, 3);
	glDrawArrays(GL_TRIANGLE_STRIP, startOcta + 6, 3);
	glDrawArrays(GL_TRIANGLE_STRIP, startOcta + 9, 3);
	glDrawArrays(GL_TRIANGLE_STRIP, startOcta + 12, 3);
	glDrawArrays(GL_TRIANGLE_STRIP, startOcta + 15, 3);
	glDrawArrays(GL_TRIANGLE_STRIP, startOcta + 18, 3);
	//glDrawArrays(GL_TRIANGLE_STRIP, startOcta + 21, 3);
}

void
drawTetra(void)
{
	glDrawArrays(GL_TRIANGLE_STRIP, startTetra, 3);
	glDrawArrays(GL_TRIANGLE_STRIP, startTetra + 3, 3);
	glDrawArrays(GL_TRIANGLE_STRIP, startTetra + 6, 3);
	glDrawArrays(GL_TRIANGLE_STRIP, startTetra + 9, 3);
}

void
display( void )
{
	onePS->particles->drawGeometry = &drawCube;
	twoPS->particles->drawGeometry = &drawTetra;
	threePS->particles->drawGeometry = &drawOcta;

    glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);     // clear the window
	vec4 eye = /*RotateZ(x_view) * RotateY(-theta) * */vec4(0.0, 0.0, 10.0, 1.0);
	...
	twoPS->env(&eye, &view, &projection);
	twoPS->draw();

	threePS->env(&eye, &view, &projection);
	threePS->draw();


	//Left Controlled Camera
	eye = /*RotateZ(x_view) * RotateY(-theta) * */vec4(x_view - 4, y_view, zoom, 1.0);
